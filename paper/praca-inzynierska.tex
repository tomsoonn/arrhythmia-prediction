...
Predykcja równie¿ inicjowana jest w innym w±tku i odbywa siê co 1 sekundê w trakcie odtwarzania wykresu, o ile liczba danych jest ju¿ wystarczaj±ca dla wybranego modelu predykcji.
...
W ramach predykcji klasyfikujemy trzy rodzaje arytmii. Klasyfikacjê nieznanych uderzeñ (Q) pominêli¶my ze wzglêdu na zbyt ma³± ilo¶æ danych, co jest zgodne ze standardem AAMI. Predykcjê traktujemy jako uderzenie normalne wówczas gdy sieæ nie przewiduje wyst±pienia ¿adnej arytmii. Rodzaje uderzeñ, które uwzglêdniamy to:
\begin{itemize}
...
W tym rozdziale opisujemy wyniki najwa¿niejszych testowanych przez nas modeli. Najlepsze z nich zosta³y wybrane do ostatecznej wersji aplikacji.
...
\begin{table}[h]
\centering
\begin{tabular}{p{4em}lllll}
Rozmiary warstw & Skuteczno¶æ & MSE & Precyzja & Czu³o¶æ & Miara $F_1$ \\ \hline
10, 50, 10      & \SI{67.04}{\percent} & $0.2102$ & $0.4789$ & $0.3681$ & $0.3917$ \\
100, 100        & \SI{68.20}{\percent} & $0.2086$ & $0.4631$ & $0.3754$ & $0.3973$ \\
100, 200, 100   & \SI{69.00}{\percent} & $0.1969$ & $0.4926$ & $0.3370$ & $0.3836$
\end{tabular}
\caption{Podsumowanie wyników dla sieci gêstych}
\end{table}
...
\end{tabular}
\caption{Podsumowanie wyników dla sieci konwolucyjnych}
\end{table}

\begin{lstlisting}[float=h!, style=result, caption={Wyniki sieci konwolucyjnej z rozmiarem 'poolingu' 16, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 4, 16}]
...
\begin{figure}[H]
	...
	\caption{Wykres straty dla sieci konwolucyjnej z rozmiarem 'poolingu' 16, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 4, 16}
...
\begin{figure}[H]
	...
	\caption{Wykres dok³adno¶ci dla sieci konwolucyjnej z rozmiarem 'poolingu' 16, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 4, 16}
	\label{fig:conv_16_4_acc}
\end{figure}

\begin{lstlisting}[float=h!, style=result, caption={Wyniki sieci konwolucyjnej z rozmiarem 'poolingu' 16, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 32, 16}]
...
\begin{figure}[H]
	...
	\caption{Wykres straty dla sieci konwolucyjnej z rozmiarem 'poolingu' 16, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 32, 16}
...
\begin{figure}[H]
	...
	\caption{Wykres dok³adno¶ci dla sieci konwolucyjnej z rozmiarem 'poolingu' 16, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 32, 16}
	\label{fig:conv_16_32_acc}
\end{figure}

\begin{lstlisting}[float=h!, style=result, caption={Wyniki sieci konwolucyjnej z rozmiarem 'poolingu' 8, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 16, 8, 4}]
...
\begin{figure}[H]
	...
	\caption{Wykres straty dla sieci konwolucyjnej z rozmiarem 'poolingu' 8, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 16, 8, 4}
...
\begin{figure}[H]
	...
	\caption{Wykres dok³adno¶ci dla sieci konwolucyjnej z rozmiarem 'poolingu' 8, rozmiarem filtrów 16 i ilo¶ciami filtrów w warstwach: 16, 8, 4}
...
W ramach tej pracy in¿ynierskiej uda³o nam siê stworzyæ aplikacjê desktopow± spe³niaj±c± za³o¿one wymagania. W programie dostêpne s± 4 modele predykcji: 2 oparte o sieci gêste oraz 2 oparte o sieci konwolucyjne. Zarówno interfejs jaki i modele predykcji spe³niaj± swoje zadanie. Uda³o nam siê równie¿ zbadaæ skuteczno¶æ predykcji arytmii przez ró¿ne modele predykcji. Projekt dostarcza równie¿ spore mo¿liwo¶ci je¿eli chodzi o rozwijanie i udoskonalania. 
